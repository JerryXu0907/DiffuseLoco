...
class CyberPushDoorEnv(CyberEnv):
    ...
    def post_physics_step(self):
        ...
        # if len(env_ids) > 0:
        #     success = torch.logical_and(self.root_states[env_ids, 0] - self.env_origins[env_ids, 0] > 1.5, self.door_dof_pos[env_ids, 0] > np.pi / 3)
        #     print("success", success)
        if self.cfg.mode == "test":
            for env_id in env_ids:
                if self.env_finish_buffer[env_id] < 11:
                    if self.env_finish_buffer[env_id] > 0:
                        self.env_success_buffer[env_id] += torch.logical_and(self.root_states[env_id, 0] - self.env_origins[env_id, 0] > 1.5, self.door_dof_pos[env_id, 0] > np.pi / 3)
                    self.env_finish_buffer[env_id] += 1
                    # if self.episode_length_buf[env_id] < 30:
                    #     self.env_finish_buffer[env_id] -= 1 # 80.7% 80.05%
            ...
            #     print("success: {}, finish: {}, rate: {}".format(self.success_count, self.finish_count, self.success_count / self.finish_count))
